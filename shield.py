...
def get_params():
        ...
        opts,args = getopt.getopt( sys.argv[1:], '', [ 'unspent=', 'fee=', 'hush-cli=', 'verbosity=', 'help' ] )
	...
	for x in opts:
        ...
        params['hush-cli'] = params['hush-cli']  if 'hush-cli' in params else './src/hush-cli'
...
def process_unspent( params ):
	...
	unspent = hush_cli_rpc( 'listunspent' )
	showresult( unspent )
...
def hush_cli( argstr ):
	...
	params = get_params()
        cmd = '%s %s' % ( params['hush-cli'], argstr )
        myecho( "\nexecuting: %s\n\n" % (cmd), 'debug' )
...
def hush_cli_rpc( command, args=None ):
        call = command + " "
        if( args != None ):
                for v in args:
                        call = call + escapeshellarg(v) + ' '
        return hush_cli( call )
...
keyinfo = hush_cli_rpc( 'zcrawkeygen' )
showresult( keyinfo )
...
rawtx = hush_cli_rpc( 'createrawtransaction', [txbuf, '{}'] )
showresult( rawtx )

# send to our private hush address

showheader( "Calling zcrawjoinsplit" )
fee = params['fee']
param = { keyinfo['zcaddress'] : total - fee }
pour = hush_cli_rpc( 'zcrawjoinsplit', [rawtx, '{}', json_encode( param ), total, fee] )
showresult( pour )

# Sign the Tx
showheader( "Signing Tx" )
sig = hush_cli_rpc( 'signrawtransaction', [pour['rawtxn']] )
showresult( sig )


# Send the Tx to network
showheader( "Sending the Tx to ourself" )
tx = hush_cli_rpc( 'sendrawtransaction', [sig['hex']] )
showresult( tx )


# Decrypt received Tx
showheader( "Decrypting the received Tx" )
received = hush_cli_rpc( 'zcrawreceive', [keyinfo['zcsecretkey'], pour['encryptednote1']] )
showresult( received )
